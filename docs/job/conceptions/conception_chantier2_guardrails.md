...
| **Classifieur de modération**      | Détection **sémantique** haine/violence/sexuel — **Detoxify (local, auto-hébergé)**, séparé de l'agent qui répond                                                                       |
...
| ------------------------------ | :-------------------------------------------------: | :----: | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| **G1** Haine/harcèlement       |                         ✅                          |   ✅   | Classifieur de modération (Detoxify, local)                                                    | Refus poli générique + log `guardrail_audit`                                                                  |
...
| **G4** PII sensible en sortie  |                    — (voir note)                    |   ✅   | Regex + validation (Luhn pour cartes, motifs mot de passe/JWT) + cross-check `user_id` mémoire | Réponse **filtrée** (donnée masquée `••••`) + log ; pas de refus total si le reste de la réponse est utile    |
...
| **G6** Injection de prompt     |                         ✅                          |   ✅   | Motifs heuristiques (« ignore tes instructions »…) + LLM-juge de cohérence                     | Refus poli **neutre** (ne confirme pas la détection) + log avec sévérité                                      |
...
**G4 (PII en sortie) fait exception** : regex + validation Luhn sont **déterministes**, pas de score — une chaîne est un numéro de carte valide ou ne l'est pas. Pas de zone grise, donc pas de seuil à calibrer pour cette catégorie.
...
| ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Classifieur de modération : API ou auto-hébergé ?                 | **Auto-hébergé** (Detoxify)                                                                                                                                                       | Coût zéro, aucune clé API à gérer, tâche bien cernée (texte court) où un petit modèle local suffit.                                     |
