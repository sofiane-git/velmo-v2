...
| **Azure AI Content Safety — Prompt Shields** | Détection cloud **dédiée** à l'injection de prompt/jailbreak, complète le LLM-juge sur **G6** — *hors* le volet modération (haine/violence/sexuel) de Content Safety, qui reste sur Detoxify (local, décision déjà actée) |
...
| Adopter Content Safety pour la modération (G1/G2/G3) aussi ?      | **Non** — reste sur Detoxify (local)                                                                                                                                               | Ferait doublon avec une brique déjà gratuite et jugée suffisante ; pas de gain de recall démontré qui justifie le coût/latence cloud supplémentaire.                                     |
