...
| **G5 · G6** (hors-périmètre, injection) | **Fail-closed** : la panne du juge applique un **blocage** (refus poli) sur sa catégorie (`pipeline._fallback_hits`, D4-01) ; le `RuleBasedJudge` déterministe tourne en **shadow permanent** (exercé/mesuré, voir spec ci-dessous), pas comme décision live. | G6 : une injection qui passe peut détourner l'agent. **G5** : un avis juridique/médical hors mandat émis par erreur est un risque de **responsabilité légale** pour Velmo, pas seulement un flag manqué — disproportionné face au coût d'un refus temporaire. |
